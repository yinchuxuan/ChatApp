...
Display rules 定义 assistant 消息进入普通对话流渲染前的 UI-only 变换。
...
- 第一版只处理普通对话流中的 assistant content，不处理 `_thinking`。
...
`display.assistant` 中的规则按数组顺序执行。每条规则的输出作为下一条规则的输入。
...
| stage | 输入 | 输出 | 用途 |
|---|---|---|---|
| `before_markdown` | assistant 原始 `content` | Markdown 源文本 | 隐藏标签、包装文本约定、轻量格式增强 |
