...
- `performance.proficiency`：学园祭演出熟练度，数值范围为 0～100。只有实际发生足以影响演出状态的演出练习、磨合、失误或状态波动时才变化，单轮增减不超过 5。
