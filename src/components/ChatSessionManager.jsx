function ChatSessionManager({ onBeforeSessionChange, onSessionChanged }) {
  ...
  const renderSession = (session) => C('div', { key: session.id, className: `chat-session-row${session.id === activeId ? ' active' : ''}`, onClick: (event) => { event.stopPropagation(); activate(session.id); } },
    C('span', { className: 'chat-session-row-state', 'aria-hidden': 'true' }),
    ...
    editingId === session.id ? C('button', { className: 'chat-session-action', onClick: (event) => renameSession(event, session.id), title: '保存会话名', 'aria-label': '保存会话名' }, C('span', { className: 'material-icons' }, 'check')) :
      C('button', { className: 'chat-session-action', onClick: (event) => { event.stopPropagation(); setEditingId(session.id); setDraftTitle(session.title || session.id); }, title: '重命名会话', 'aria-label': '重命名会话' }, C('span', { className: 'material-icons' }, 'edit')),
    C('button', { className: 'chat-session-action danger', onClick: (event) => deleteSession(event, session.id), title: '删除会话', 'aria-label': '删除会话' }, C('span', { className: 'material-icons' }, 'delete'))
  );

  return C('div', { className: 'chat-session-manager' },
    C('button', { className: 'chat-session-btn', onClick: (event) => { event.stopPropagation(); setOpen(!open); if (!open) loadSessions(); }, title: '管理聊天会话', 'aria-label': '管理聊天会话' }, C('span', { className: 'material-icons' }, 'inventory_2')),
