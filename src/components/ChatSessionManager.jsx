function ChatSessionManager({ onBeforeSessionChange, onSessionChanged }) {
  ...
  const renderSession = (session) => C('div', { key: session.id, className: `chat-session-row${session.id === activeId ? ' active' : ''}`, onClick: (event) => { event.stopPropagation(); activate(session.id); } },
    C('span', { className: 'material-icons chat-session-row-icon' }, session.id === activeId ? 'radio_button_checked' : 'radio_button_unchecked'),
    C('div', { className: 'chat-session-row-main' },
      ...
      }) : C('div', { className: 'chat-session-title' }, session.title || session.id),
      C('div', { className: 'chat-session-preview' }, session.preview || `${session.messageCount || 0} 条消息`)
    ),
    editingId === session.id ? C('button', { className: 'md-btn md-btn-icon chat-session-action', onClick: (event) => renameSession(event, session.id), title: '保存会话名', 'aria-label': '保存会话名' }, C('span', { className: 'material-icons' }, 'check')) :
      C('button', { className: 'md-btn md-btn-icon chat-session-action', onClick: (event) => { event.stopPropagation(); setEditingId(session.id); setDraftTitle(session.title || session.id); }, title: '重命名会话', 'aria-label': '重命名会话' }, C('span', { className: 'material-icons' }, 'edit')),
    C('button', { className: 'md-btn md-btn-icon chat-session-action danger', onClick: (event) => deleteSession(event, session.id), title: '删除会话', 'aria-label': '删除会话' }, C('span', { className: 'material-icons' }, 'delete'))
  );
  ...
  return C('div', { className: 'chat-session-manager' },
    ...
    open ? C('div', { className: 'chat-session-panel', onClick: (event) => event.stopPropagation() },
      C('div', { className: 'chat-session-panel-head' },
        ...
        C('button', { className: 'md-btn md-btn-icon chat-session-action', onClick: createSession, disabled: busy, title: '新建会话', 'aria-label': '新建会话' }, C('span', { className: 'material-icons' }, 'add'))
      ),
      C('div', { className: 'chat-session-list' }, sessions.map(renderSession))
    ) : null
  );
}
